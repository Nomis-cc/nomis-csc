...
namespace Nomis.CscExplorer
{
        ...
        ICscExplorerClient
    {
            ...
            where TRecord : ICscExplorerTransferRecord
        {
            ...
            var page = 1;
            var transactionsData = await GetTransactionList<TResult, TData, TRecord>(address);
            result.AddRange(transactionsData.Data?.Records ?? new List<TRecord>());
            while (transactionsData?.Data?.HasNext == true)
            {
                page++;
                transactionsData = await GetTransactionList<TResult, TData, TRecord>(address, page);
                result.AddRange(transactionsData.Data?.Records ?? new List<TRecord>());
            }
        ...
        private async Task<TResult> GetTransactionList<TResult, TData, TRecord>(
            string address,
            int page = 1)
            where TResult : ICscExplorerTransferList<TData, TRecord>
            where TData : ICscExplorerTransferData<TRecord>
